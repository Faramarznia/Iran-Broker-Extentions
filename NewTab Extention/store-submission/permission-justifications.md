...
> Not currently required — see note below before submitting.

**نسخه فارسی (برای مرور داخلی تیم):**

⚠️ **یافتهٔ مهم، نه یک توجیهِ آماده:** با جست‌وجوی کامل `chrome.storage.*` در تمام ۸ فایل اسکریپت زنده (`newtab.js` + ۷ فایل `js/`)، **هیچ فراخوانی به `chrome.storage.local` یا `chrome.storage.sync` پیدا نشد**. تمام ذخیره‌سازی محلی پروژه (تنظیمات، تاریخچهٔ چت آیرو، ژورنال معاملاتی، Hub، فوکوس‌مود و…) از `localStorage` استاندارد استفاده می‌کند — که یک Web API معمولی است و اصلاً به permission `storage` نیاز ندارد (هر صفحهٔ وب معمولی هم بدون هیچ مجوزی به آن دسترسی دارد).

یعنی این permission در حال حاضر **کاملاً بلااستفاده** است. دو گزینه:
1. **توصیه‌شده:** آن را از `manifest.json` حذف کنید — یک permission کمتر یعنی متن Justification کمتری لازم است و سطح حمله/سؤال ریویو Chrome Web Store پایین‌تر می‌آید. این دقیقاً همان الگویی است که برای `host_permissions` بلااستفاده در ممیزی قبلی همین پروژه اعمال شد.
2. اگر برنامه‌ای برای مهاجرت از `localStorage` به `chrome.storage.local` در آینده نزدیک دارید (مثلاً برای همگام‌سازی بین دستگاه‌ها با `chrome.storage.sync`)، آن را نگه دارید ولی در یادداشت داخلی مستند کنید که «رزرو‌شده، فعلاً بلااستفاده» — دقیقاً مثل کاری که برای `api.anthropic.com` در `DATA-SOURCES.md` انجام شده.

**این مورد را در جدول «تصمیمات باز» در `README.md` همین پوشه هم می‌بینید.**
...
| `api.gapgpt.app` | بک‌اند فعال دستیار «آیرو» — پردازش پیام‌های چت | سرویس فعلی انتخاب‌شده برای این ویژگی |
| `api.anthropic.com` | مسیر جایگزین **غیرفعال** برای آیرو (`ACTIVE = 'gapgpt'` هاردکد در `js/airo.js`، هرگز در runtime عوض نمی‌شود) | نگه‌داشته شده تا سوییچ بین دو سرویس بدون درخواست permission جدید و ریلیز اضطراری ممکن باشد؛ در صورت تصمیم به حذف این قابلیت جایگزین، این خط هم قابل حذف است |
