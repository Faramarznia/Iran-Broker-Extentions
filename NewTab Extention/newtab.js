...
(function () {
  ...
  function initTelegramSidebar() {
    ...
    var reloadBtn = document.getElementById('sb-tg-reload');
    var winBtn = document.getElementById('sb-tg-window');
    var diagEl = document.getElementById('sb-tg-diag');
    ...
    // Reliable fallback: open Telegram in a docked side window (no iframe, no
    // X-Frame-Options problem; login persists in the normal browsing context).
    if (winBtn) {
      winBtn.addEventListener('click', function () {
        try {
          chrome.windows.create({
            ...
            top: 0,
            width: 480,
            height: (window.screen && window.screen.availHeight) || 900
          });
        } catch (e) {
    ...
    // In-panel diagnostic: definitively reports whether the un-frame DNR rule is
    // registered and whether it actually matched the Telegram iframe request —
    // so we can tell "rule not firing" from "browser refuses to strip headers".
    function runDiag() {
      if (!diagEl) return;
      if (!(window.chrome && chrome.runtime && chrome.runtime.sendMessage)) {
        diagEl.textContent = 'API اکستنشن در دسترس نیست — اکستنشن کامل ری‌لود نشده';
        return;
      }
      try {
        chrome.runtime.sendMessage({ type: 'tg-diag' }, function (resp) {
          if (chrome.runtime.lastError || !resp) {
            diagEl.textContent = 'سرویس‌ورکر اجرا نشده — اکستنشن را از chrome://extensions کامل ری‌لود کن';
            return;
          }
          if (resp.apiInSw === false) {
            diagEl.textContent = 'پرمیشن declarativeNetRequest فعال نشده — اکستنشن را حذف و دوباره Load unpacked کن';
            return;
          }
          if (resp.error) { diagEl.textContent = 'خطا: ' + resp.error; return; }
          diagEl.textContent = 'قانون حذف هدر: ' + (resp.registered ? 'ثبت‌شده ✓' : 'ثبت‌نشده ✗') +
            '\nاجرای قانون روی درخواست‌ها: ' + (resp.matched ? (resp.matched + ' بار ✓') : 'هیچ ✗');
        });
      } catch (e) {
        diagEl.textContent = 'خطا در ارتباط: ' + String(e);
      }
    }
    if (frame) {
    ...
    function setSrc() {
      if (!frame) return;
      if (frameWrap) frameWrap.classList.remove('loaded');
      // Cache-bust: DNR header modification is skipped for responses served
      // from disk cache (the cached copy keeps X-Frame-Options: deny), so force
      // a fresh network fetch each load to let the un-frame rule strip headers.
      frame.setAttribute('src', TG_URL + '?_ib=' + Date.now());
    }
    function load() {
      // lazy first load: only set src once
      if (frame && !frame.getAttribute('src')) setSrc();
